...
class AdCreatorService:
    ...
    async def analyze_and_select_creatives(self, config: Dict[str, Any], 
        ...
        Returns:
            Análise completa de criativos e seleção dos melhores
        """
        logger.info("🎨 FASE 5: Analisando criativos...")
        ...
        try:
            ...
            if uploaded_files and len(uploaded_files) > 0:
                logger.info(f"📁 Analisando {len(uploaded_files)} arquivos enviados...")
                
                for file_info in uploaded_files:
                    creative_analysis = await self._analyze_creative(file_info)
                    results['uploaded_creatives'].append(creative_analysis)
                
                logger.info(f"✅ {len(results['uploaded_creatives'])} criativos analisados")

            # Gerar criativos via IA se necessário
            if self.creative_intel and (not uploaded_files or len(uploaded_files) < 3):
                logger.info("🤖 Gerando criativos via IA...")
                ...
                try:
                    ...
                    logger.info(f"✅ {len(generated)} criativos gerados")
                except Exception as e:
            ...
            if all_creatives:
                ...
                logger.info(f"✅ {len(selected)} criativos selecionados")
    ...
    async def _analyze_creative(self, file_info: Dict) -> Dict[str, Any]:
        """Analisar um criativo específico."""
        return {
            'file_name': file_info.get('name'),
            'file_type': file_info.get('type'),
            'file_size': file_info.get('size'),
            'quality_score': 85,  # Será calculado via IA
    ...
    async def _generate_creatives_ai(self, product_info: Dict, platform: str, count: int) -> List[Dict]:
        ...
        for i in range(count):
            creatives.append({
                ...
                'ai_generated': True,
                'style': 'modern',
                'description': f'Criativo gerado pela IA - Variação {i+1}'
            })
    ...
    def _generate_creative_recommendations(self, results: Dict) -> List[str]:
        """Gerar recomendações sobre criativos."""
        recommendations = []
        
        total_creatives = len(results['selected_creatives'])
        
        if total_creatives >= 3:
            recommendations.append(f"✅ Ótimo! {total_creatives} criativos selecionados para testes A/B")
        else:
            recommendations.append(f"⚠️ Poucos criativos ({total_creatives}). Recomendamos pelo menos 3 para testes eficazes")
        
        if results['generated_creatives']:
            recommendations.append(f"🤖 IA gerou {len(results['generated_creatives'])} criativos adicionais")
        
        return recommendations


    # ================================================================
    # FASE 6: Criação Automática da Estratégia
    # ================================================================
    ...
    async def create_campaign_strategy(self, config: Dict[str, Any], 
        ...
        """
        FASE 6: Criar estratégia completa de campanha.
        
        Args:
            config: Configuração do usuário
            analysis_results: Resultados da análise (FASE 3-4)
            creative_results: Resultados dos criativos (FASE 5)
        
        Returns:
            Estratégia completa de campanha
        """
        logger.info("🎯 FASE 6: Criando estratégia de campanha...")
        ...
        try:
            ...
            if self.budget_calc:
                try:
                    ...
                    logger.error(f"Erro na alocação de orçamento: {e}")
            ...
            logger.error(f"❌ Erro na FASE 6: {e}")
    ...
    def _define_campaign_objective(self, platform: str, product_analysis: Dict) -> Dict[str, Any]:
        ...
        if platform == 'meta':
            ...
            return {
                ...
                'description': 'Maximizar valor de conversão'
            }
    ...
    def _create_campaign_structure(self, platform: str, creatives_count: int) -> Dict[str, Any]:
        ...
        return {
            'campaigns': 1,
            'ad_sets': 3,  # Teste de 3 públicos diferentes
            'ads_per_set': min(creatives_count, 3),  # Máximo 3 anúncios por conjunto
    ...
    async def _allocate_budget(self, total_budget: float, budget_type: str,
        ...
        """Alocar orçamento entre conjuntos de anúncios."""
        ad_sets_count = campaign_structure.get('ad_sets', 3)
        
        if budget_type == 'daily':
            budget_per_set = total_budget / ad_sets_count
        else:
            # Orçamento total - dividir por 7 dias e depois por ad sets
    ...
    def _create_schedule(self, operation_mode: str) -> Dict[str, Any]:
        """Criar cronograma de veiculação."""
        return {
            'start_date': datetime.utcnow().isoformat(),
            'end_date': None,  # Contínuo
            'dayparting': None if operation_mode == 'normal' else {
                'enabled': True,
                'best_hours': [9, 10, 11, 18, 19, 20, 21]  # Horários de pico
            },
    ...
    # ================================================================
    # FASE 7: Criação Automática dos Anúncios
    # ================================================================
    ...
    async def create_ads_automatically(self, config: Dict[str, Any],
                                      ...
                                      creative_results: Dict) -> Dict[str, Any]:
        """
        FASE 7: Criar anúncios automaticamente com copies e criativos.
        
        Args:
            config: Configuração do usuário
            strategy: Estratégia da campanha (FASE 6)
            creative_results: Resultados dos criativos (FASE 5)
        
        Returns:
            Anúncios criados e prontos para publicação
        """
        logger.info("📝 FASE 7: Criando anúncios automaticamente...")
        ...
        try:
            ...
            if not selected_creatives:
                raise ValueError("Nenhum criativo selecionado para criar anúncios")

            # Gerar copies para os anúncios
            logger.info("✍️ Gerando copies via IA...")
            copies = await self._generate_ad_copies(
                product_info=config.get('product_analysis'),
                platform=config.get('platform'),
                count=len(selected_creatives) * 3  # 3 variações por criativo
            )

            # Criar combinações de copy + criativo
            logger.info("🔀 Criando combinações de anúncios...")
            ...
            # Criar variações A/B
            logger.info("🧪 Criando variações A/B...")
            ...
            results['status'] = 'completed'
            logger.info(f"🎉 FASE 7 concluída! {len(ads)} anúncios criados")
            ...
            logger.error(f"❌ Erro na FASE 7: {e}")
            results['status'] = 'error'
            results['error'] = str(e)
            return results

    async def _generate_ad_copies(self, product_info: Dict, platform: str, count: int) -> List[Dict]:
        """Gerar copies de anúncios via IA."""
        copies = []
        
        # Usar Ad Copy Generator se disponível
        if self.ad_copy_gen:
            try:
                for i in range(count):
                    copy = {
                        'id': f'copy_{i+1}',
                        'headline': f'Transforme Seu Negócio Hoje - Variação {i+1}',
                        'primary_text': f'Descubra a solução perfeita para {product_info.get("category", "seu negócio")}. Resultados garantidos!',
    ...
    def _get_default_copies(self, count: int) -> List[Dict]:
        """Copies padrão caso IA não esteja disponível."""
        templates = [
            {
                'headline': 'Oferta Exclusiva - Não Perca!',
                'primary_text': 'Produto de alta qualidade com entrega rápida',
                'description': 'Compre agora e ganhe desconto',
                'call_to_action': 'SHOP_NOW'
            },
            {
                'headline': 'Solução Perfeita Para Você',
            ...
            },
            {
                ...
                'primary_text': 'Qualidade garantida e preço justo',
    ...
    def _create_ad_combinations(self, creatives: List[Dict], copies: List[Dict],
        ...
        # Limitar a 9 anúncios (3 criativos x 3 copies)
        ...
        for i, creative in enumerate(creatives[:max_creatives]):
            for j, copy in enumerate(copies[i*max_copies_per_creative:(i+1)*max_copies_per_creative]):
                ad = {
                    'id': f'ad_{i+1}_{j+1}',
                    'name': f'Anúncio {i+1}.{j+1}',
                    'creative': creative,
                    'copy': copy,
                    'platform': platform,
                    'status': 'draft',
                    'editable': True,  # IMPORTANTE: Permitir edição
                    'preview_url': None,
    ...
    def _create_ab_test_variations(self, ads: List[Dict]) -> List[Dict]:
        ...
        # Agrupar anúncios em pares para teste A/B
    ...
    def _estimate_reach(self, strategy: Dict) -> int:
        """Estimar alcance baseado na estratégia."""
    ...
    def _estimate_cpa(self, strategy: Dict) -> float:
        """Estimar CPA baseado na estratégia."""
        return 25.0  # Estimativa padrão


    # ================================================================
    # FASE 9: Execução e Monitoramento Contínuo
    # ================================================================
    ...
    async def execute_campaign(self, ads: List[Dict], config: Dict) -> Dict[str, Any]:
        ...
        Args:
            ads: Lista de anúncios aprovados
            config: Configuração da campanha
        
        Returns:
            Resultado da execução e IDs das campanhas criadas
        """
        logger.info("🚀 FASE 9: Executando campanha...")
        ...
        try:
            ...
            result['status'] = 'live'
            logger.info(f"🎉 FASE 9 concluída! {len(result['ad_ids'])} anúncios publicados")
            ...
            logger.error(f"❌ Erro na FASE 9: {e}")
            result['status'] = 'error'
            result['error'] = str(e)
            return result
    ...
    async def _execute_on_meta(self, ads: List[Dict], config: Dict) -> Dict[str, Any]:
        ...
        logger.info("📱 Publicando no Meta Ads...")

        # Simular publicação (integração real será feita posteriormente)
        ...
        return {
            ...
            'logs': [
                ...
                f"{len(ad_ids)} anúncios publicados",
                "Status: Ativo"
            ]
        }
    ...
    async def _execute_on_google(self, ads: List[Dict], config: Dict) -> Dict[str, Any]:
        ...
        logger.info("🔍 Publicando no Google Ads...")
    ...
    async def _start_monitoring(self, campaign_ids: List[str], operation_mode: str):
        """Iniciar monitoramento contínuo das campanhas."""
        logger.info("👁️ Iniciando monitoramento contínuo...")

        # Configurar monitoramento baseado no modo de operação
        monitoring_config = {
            'campaign_ids': campaign_ids,
        ...
        # Registrar configuração de monitoramento
    ...
    async def turbo_mode_optimizer(self, campaign_ids: List[str]) -> Dict[str, Any]:
        """
        FASE 9.1: Otimização agressiva em modo Turbo.
        
        - Testes rápidos (2h)
        - Cortes agressivos (ROAS < 1.5)
        - Escala automática (ROAS > 3)
        - Budget reallocation
        """
        logger.info("⚡ TURBO MODE: Otimizando agressivamente...")
        ...
        try:
            ...
            logger.info(f"⚡ TURBO MODE concluído: {result['ads_scaled']} escalados, {result['ads_paused']} pausados")

            return result

        except Exception as e:
            logger.error(f"❌ Erro no TURBO MODE: {e}")
            result['status'] = 'error'
            result['error'] = str(e)
            return result
    ...
    async def _pause_ad(self, ad_id: str):
        """Pausar anúncio."""
        logger.info(f"⏸️ Pausando anúncio: {ad_id}")

    async def _scale_ad(self, ad_id: str, multiplier: float):
        """Escalar orçamento do anúncio."""
        logger.info(f"📈 Escalando anúncio {ad_id} em {multiplier}x")

    # ================================================================
    # FASE 10: Monitoramento Contínuo Inteligente
    # ================================================================

    async def intelligent_monitoring(self, campaign_ids: List[str]) -> Dict[str, Any]:
        """
        FASE 10: Monitoramento contínuo com insights da IA.
        
        Returns:
            Dashboard em tempo real com recomendações
        """
        logger.info("🧠 Monitoramento inteligente ativo...")
    ...
    async def _analyze_with_ai(self, metrics: Dict) -> List[str]:
        ...
        if avg_roas > 3.0:
            insights.append("🎉 Excelente performance! ROAS acima da meta.")
        elif avg_roas > 2.0:
            insights.append("✅ Performance boa. Considere escalar gradualmente.")
        else:
            insights.append("⚠️ Performance abaixo do esperado. Revisar segmentação e criativos.")

        return insights

    def _calculate_health_score(self, metrics: Dict) -> int:
        """Calcular score de saúde da campanha (0-100)."""
        # Lógica simplificada
        avg_roas = sum(m.get('roas', 0) for m in metrics.values()) / len(metrics)
        ...
        for ad_id, ad_metrics in metrics.items():
            if ad_metrics.get('roas', 0) < 1.0:
                alerts.append({
                    'ad_id': ad_id,
                    'type': 'critical',
                    'message': f'ROAS crítico: {ad_metrics.get("roas")}',
    ...
    # ================================================================
    # FASE 11: Relatórios e Inteligência Acumulada
    # ================================================================

    async def generate_intelligence_report(self, campaign_ids: List[str], 
                                          timeframe: str = '7d') -> Dict[str, Any]:
        """
        FASE 11: Gerar relatório com inteligência acumulada.
        
        Args:
            campaign_ids: IDs das campanhas
            timeframe: Período do relatório
        
        Returns:
            Relatório completo com aprendizados
        """
        logger.info("📊 Gerando relatório de inteligência...")
    ...
    def _extract_learnings(self, data: Dict) -> List[str]:
        ...
        return [
            ...
            "Horário noturno (18h-22h) tem melhor conversão",
            "Público Lookalike 1% tem ROAS 2x maior",
            "Headlines com números aumentam CTR em 25%"
        ]
    ...
    def _generate_best_practices(self, learnings: List[str]) -> List[str]:
        ...
        return [
            ...
            "Concentrar orçamento no horário noturno",
